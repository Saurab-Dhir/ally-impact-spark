...
const MetricsGrid = () => {
  ...
  const metrics: { [key: string]: Category } = {
    prevention: {
      title: "Prevention",
      color: "prevention",
      icon: "🛡️",
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {Object.entries(metrics).map(([key, category]) => (
          <div
            key={key}
            className="bg-white rounded-2xl shadow-card overflow-hidden border border-slate-100 hover:shadow-soft transition-all duration-300"
          >
            {/* Category Header */}
            <div className={`impact-card-${category.color} px-6 py-4`}>
              <div className="flex items-center gap-3">
                <span className="text-2xl">{category.icon}</span>
                <div>
                  <h3 className="text-white font-bold text-lg">{category.title}</h3>
                  <p className="text-white/80 text-sm">Anti-Trafficking Impact</p>
                </div>
              </div>
            </div>
